...
function filterOffensiveComments(commentsArray, bannedWords) {
  let tabOut = commentsArray.filter(function(item){
    for(let i=0 ; i<bannedWords.length ; i++) { 
      if((item.toLowerCase()).includes(bannedWords[i].toLowerCase())){
        return null;
      }
    }
    return item;
