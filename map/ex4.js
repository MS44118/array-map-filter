...
function getCirclesProperties(radiusesArray) {
  let tabOut = radiusesArray.map(function(radiusesObject){
    const obj = {
      radius: radiusesObject, //number
      circumference: (2 * Math.PI * radiusesObject).toFixed(3) , //string
      surface: (Math.PI * radiusesObject * radiusesObject).toFixed(3) //string
    };
    return obj;
  })
  return tabOut;
